...
在批量备份场景下点击取消按钮后,虽然正在上传的文件的分块上传被正确中止,但已经完成上传的文件(如 `apk.tar.zst`、`user.tar.zst`、`user_de.tar.zst`)未被删除,导致存储桶中残留不完整的备份数据。

#### 根本原因

1. **状态判断时序问题**:
    - 原清理逻辑使用 `pkg.state != OperationState.DONE` 判断是否需要清理
    - 但数据上传完成后状态立即变为 `DONE`,而整个备份流程(包括配置保存)还未完成
    - 导致清理逻辑误判为"已完成"而跳过删除

2. **S3 客户端生命周期问题**:
    - 在清理时 `s3Client` 已被断开连接(endpoint 为 null)
    - 导致 `listObjectsV2` 调用失败,返回空列表
    - 无法列出需要删除的对象
...
- 将判断条件从 `if (pkg.state != OperationState.DONE)` 改为 `if (index >= currentIndex - 1)`
- 使用索引位置判断,确保清理"正在处理的包"及之后的所有包
- 避免依赖状态字段的时序问题

**媒体备份服务** (`BackupServiceCloudImpl.kt`):
- 同样改为 `if (index >= currentIndex - 1)` 的索引判断
- 与包备份保持一致的清理策略
